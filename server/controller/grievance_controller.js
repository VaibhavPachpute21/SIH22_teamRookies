...
exports.CreateGrievance = async (req, res, next) => {
  ...
  try {
    const firstOfficerReciever = await Officer.findOne({
      university: grievant_university,
      role: "1A",
    ...
    const newGrievance = await Grievance.create({
      ...
      reciever_id: firstOfficerReciever._id,
      grievant_university: grievant_university,
      imgs: imgs,
      grievance_nature: grievance_nature,
    ...
    console.log(
      `Grievance by ${grievant_name} was forwarded to ${firstOfficerReciever.fullname}`
    );
...
exports.EditGrievance = async (req, res, next) => {
  ...
  try {
    const updatedGrievance = await Grievance.findOneAndUpdate(
      { _id: id },
      { $set: { grievant_id, reciever_id, grievance_info, imgs } },
      { new: true }
    );
...
exports.SatisfiedWithReply = async (req, res, next) => {
  ...
  try {
    ...
    const updatedSolveCount = await Officer.findOneAndUpdate(
      { _id: reciever_id },
      { $inc: { solve_count: 1 } }
    );

    const upadateForwardSatisfied = await Forward.updateMany(
...
exports.GetGrievanceStatus = async (req, res, next) => {
  ...
  try {
    const latestForward = await Forward.find({ grievance_id: id })
      .limit(1)
      .sort({ $natural: -1 });
    if (!latestForward) {
      res.status(200).json({
