...
exports.IncDayCount = async (req, res, next) => {
  try {
    const Lt15 = await Grievance.updateMany(
      {
        satisfied: false,
        assigned_in_role: { $ne: "2" },
        assigned_in_role: { $ne: "X" },
        day_counter: { $lt: 15 },
      },
      { $inc: { day_counter: 1 } }
    );
      ...
      if (role2.length > 0) {
        ...
        if (regionalOfficer.length > 0) {
          ...
          await Grievance.findByIdAndUpdate(
            doc._id,
            { $set: { assigned_in_role: "X" } },
            { new: true }
          );

          console.log(
            `Grievance by ${doc.grievant_name} was forwarded to ${regionalOfficer[0]?.fullname} 1`
          );
    ...
    someData?.forEach(async (doc) => {
      ...
      if (currentOfficer) {
        const nextOfficer = await Officer.find({
          ...
          university: currentOfficer.university,
          role: doc.assigned_in_role,
        });

        if (nextOfficer.length == 0) {
          await Grievance.findByIdAndUpdate(
            doc._id,
            { $set: { assigned_in_role: "1B" } },
            { new: true }
          );
          ...
          AllGrievances?.forEach(async (doc2) => {
            const TheOfficer = await Officer.find({
              role: doc2.assigned_in_role,
              _id: doc2.reciever_id,
            });

            if (TheOfficer.length == 0) {
              const firstBOfficer = await Officer.find({
                role: doc2.assigned_in_role,
                university: doc2.grievant_university,
              }).sort("university_nodal_no");

              await Grievance.findByIdAndUpdate(doc2?._id, {
                $set: { reciever_id: firstBOfficer[0]?._id },
              });
              ...
              console.log(
                `Grievance by ${doc2.grievant_name} was forwarded to ${firstBOfficer[0]?.fullname} 2`
              );
              if (firstBOfficer[0].fullname !== undefined) {
                SendMessage(
                  doc?.grievant_name,
                  doc?.grievance_title,
                  firstBOfficer[0]?.fullname
                );

                /*  await SendEmail(firstBOfficer[0]?.fullname,doc2.grievant_name,doc2.grievance_nature)  */
              }
            } else {
              const currentOff = await Officer.find({
                role: doc2.assigned_in_role,
                _id: doc2.reciever_id,
              });
              ...
              const theNextOfficer = await Officer.find({
                university_nodal_no: nextNodalNo + 1,
                university: doc2.grievant_university,
                role: doc2.assigned_in_role,
              });
              if (theNextOfficer.length > 0) {
                await Grievance.findByIdAndUpdate(doc2._id, {
                  $set: { reciever_id: theNextOfficer[0]?._id },
                });
                ...
                console.log(
                  `Grievance by ${doc2.grievant_name} was forwarded to ${theNextOfficer[0]?.fullname} 2`
                );
                if (theNextOfficer[0].fullname !== undefined) {
          ...
          try {
            await Grievance.findOneAndUpdate(
              { reciever_id: currentOfficer._id },
              { $set: { reciever_id: nextOfficer[0]._id } }
            );
            ...
            console.log(
              `Grievance by ${doc.grievant_name} was forwarded to ${nextOfficer[0]?.fullname} 2`
            );
            if(nextOfficer[0]?.fullname !== undefined){
                SendMessage(doc.grievant_name,doc.grievance_title,nextOfficer[0]?.fullname)  
